...
echo "[INFO] Adding AWX operator Helm repository..."
helm repo add awx-operator https://ansible-community.github.io/awx-operator-helm/ || echo "[INFO] Repository already exists"
helm repo update


# Ensure namespace exists
echo "[INFO] Ensuring namespace 'awx' exists..."
kubectl create namespace awx --dry-run=client -o yaml | kubectl apply -f -

# Install AWX operator with explicit wait
echo "[INFO] Installing AWX operator..."
...
echo "[INFO] Checking for AWX operator pod..."
kubectl rollout status deployment/my-awx-operator-awx-operator -n awx --timeout=300s
